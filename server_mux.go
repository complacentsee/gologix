...
type ExplicitRequest struct {
	...
	// Route is the CIP route path that trailed an Unconnected_Send (0x52)
	// wrapper: the raw port/link segments the originator wants this message
	// forwarded along, with the leading size and reserved bytes stripped.
	//
	// A real router consumes the leading segment, forwards the rest out the
	// named port, and the message arrives at the next hop with a shorter
	// path. A handler that emulates a network of devices instead walks the
	// whole path itself to decide which device the request is addressed to.
	//
	// nil for a request that arrived without an Unconnected_Send wrapper --
	// i.e. addressed to this device directly.
	Route []byte

	// Local is the address this request arrived on. A server bound to
	// several addresses -- one per emulated device, say -- needs it to tell
	// which of them an unrouted request was addressed to.
	Local net.Addr

	// Remote is the originator's address, for logging.
	Remote net.Addr
}
...
// is the request data.
// explicitContext carries what the transport layer knows about a request that
// the EPATH itself does not: where the originator wants it delivered, and
// where its payload stops. Both matter only for Unconnected_Send; a direct
// request leaves the zero value alone.
type explicitContext struct {
	// Route is the trailing route path from an Unconnected_Send wrapper.
	Route []byte

	// DataEnd bounds the service data at the end of the embedded message.
	// Without it, Data would swallow the wrapper's own trailing route path
	// and any handler counting fixed-size records (a scattered-read reply,
	// say) would read two junk entries off the end. Zero means "to the end
	// of the item", which is right for an unwrapped request.
	DataEnd int
}
...
func (h *serverTCPHandler) dispatchExplicit(
	...
	data := item.Rest()
	if ctxt.DataEnd > 0 {
		// item.Pos is where the service data starts; clamp its end to the
		// embedded message boundary the wrapper declared.
		if ctxt.DataEnd > len(item.Data) {
			ctxt.DataEnd = len(item.Data)
		}
		if ctxt.DataEnd < item.Pos {
			ctxt.DataEnd = item.Pos
		}
		data = item.Data[item.Pos:ctxt.DataEnd]
	}
...
func (h *serverTCPHandler) connectedExplicitMux(service CIPService, items []CIPItem) error {
	...
	resp, dispatchErr := h.dispatchExplicit(service, int(pathSize), true, item, explicitContext{})
	if dispatchErr != nil {
...
func (h *serverTCPHandler) unconnectedExplicitMux(service CIPService, item *CIPItem, ctxt explicitContext) error {
	pathSize, err := item.Byte()
	if err != nil {
		return fmt.Errorf("read path size: %w", err)
	}

	resp, dispatchErr := h.dispatchExplicit(service, int(pathSize), false, item, ctxt)
	if dispatchErr != nil {
		h.server.Logger.Warn("explicit dispatch error",
			"service", service, "err", dispatchErr)
	}
